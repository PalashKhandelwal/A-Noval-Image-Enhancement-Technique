...
def improved_noise_removal(color_image):
    ...
    for y in range(1, height - 1):
        for x in range(1, width - 1):
            ...
            if np.all(center_pixel == [0, 0, 0]):  # Process only black pixels
                neighbors = color_image[y-1:y+2, x-1:x+2]

                if np.any(np.sum(neighbors, axis=2) != 0):
                    non_extreme_neighbors = neighbors[np.sum(neighbors, axis=2) != 0]
...
input_image = cv2.imread('HINR.png')
